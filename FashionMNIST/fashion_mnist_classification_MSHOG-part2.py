...
last_idx1 = 0+(image_size/2-1)**2 * N_hist_orient*4
last_idx2 = last_idx1+(image_size/4-1)**2 * N_hist_orient*4
last_idx3 = last_idx2+(30/6-1)**2 * N_hist_orient*4
...
for i in range(N_train):
	...
	feat_train[i, last_idx1:last_idx2] = hog(X_train[i,:,:], orientations=N_hist_orient, pixels_per_cell=(4, 4), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	feat_train[i, last_idx2:last_idx3] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(6, 6), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	...
	if i<N_test:
		...
		feat_test[i, 0:last_idx1] = hog(X_test[i,:,:], orientations=N_hist_orient, pixels_per_cell=(2, 2), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx1:last_idx2] = hog(X_test[i,:,:], orientations=N_hist_orient, pixels_per_cell=(4, 4), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx2:last_idx3] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(6, 6), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
