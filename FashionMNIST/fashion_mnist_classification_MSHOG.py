...
#last_idx1 = (image_size/2)**2*N_hist_orient
last_idx1 = 0+(30/3-1)**2 * N_hist_orient*4
last_idx2 = last_idx1+(image_size/4-1)**2 * N_hist_orient*4
last_idx3 = last_idx2+(30/5-1)**2 * N_hist_orient*4
last_idx4 = last_idx3+(image_size/7-1)**2 * N_hist_orient*4
last_idx5 = last_idx4+(30/10-1)**2 * N_hist_orient*4
last_idx6 = last_idx5+(image_size/14-1)**2 * N_hist_orient*4

feat_train = np.zeros((N_train,last_idx6)).astype(np.float32)
feat_test = np.zeros((N_test,last_idx6)).astype(np.float32)
block_norm = 'L2'
...
for i in range(N_train):
	...
	feat_train[i, last_idx1:last_idx2] = hog(X_train[i,:,:], orientations=N_hist_orient, pixels_per_cell=(4, 4), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	feat_train[i, last_idx2:last_idx3] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(5, 5), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	feat_train[i, last_idx3:last_idx4] = hog(X_train[i,:,:], orientations=N_hist_orient, pixels_per_cell=(7, 7), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	feat_train[i, last_idx4:last_idx5] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(10, 10), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	feat_train[i, last_idx5:last_idx6] = hog(X_train[i,:,:], orientations=N_hist_orient, pixels_per_cell=(14, 14), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
	...
	if i<N_test:
		...
		feat_test[i, 0:last_idx1] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(3, 3), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx1:last_idx2] = hog(X_test[i,:,:], orientations=N_hist_orient, pixels_per_cell=(4, 4), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx2:last_idx3] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(5, 5), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx3:last_idx4] = hog(X_test[i,:,:], orientations=N_hist_orient, pixels_per_cell=(7, 7), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx4:last_idx5] = hog(img_zp, orientations=N_hist_orient, pixels_per_cell=(10, 10), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
		feat_test[i, last_idx5:last_idx6] = hog(X_test[i,:,:], orientations=N_hist_orient, pixels_per_cell=(14, 14), 
						  cells_per_block=(2, 2), block_norm=block_norm, feature_vector=True)
...
acc = np.sum(Y_pred==Y_test)/Y_train.shape[0]
print(acc)
# pdb.set_trace()
