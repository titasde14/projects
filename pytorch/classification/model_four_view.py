...
import inception_v3_fourview_withdropoutcontrol
...
class Rescale(object):
    ...
    def __call__(self, sample):
        view1_image, view2_image, view3_image, view4_image, label = sample['view1_image'], sample['view2_image'], sample['view3_image'], sample['view4_image'], sample['label']

        c, h, w = view1_image.shape
        h = float(h)
        w = float(w)
        ...
        view3_img = np.zeros((c,new_h,new_w))
        view4_img = np.zeros((c,new_h,new_w))
        for ctr in range(0,c):
            ...
            view3_img[ctr,:,:] = transform.resize(view3_image[ctr,:,:], (new_h, new_w), mode='constant')
            view4_img[ctr,:,:] = transform.resize(view4_image[ctr,:,:], (new_h, new_w), mode='constant')

        return {'view1_image': view1_img, 'view2_image': view2_img, 'view3_image': view3_img, 'view4_image': view4_img, 'label': label}
...
class MyDataset_FourView(Dataset):
	...
	def __getitem__(self, idx):
		# print(idx)
		curr_img_view1 = self.img_array[idx][0]
		curr_img_view2 = self.img_array[idx][1]
		curr_img_view3 = self.img_array[idx][2]
		curr_img_view4 = self.img_array[idx][3]
		curr_label = self.labels_array[idx]
		sample = {'view1_image': curr_img_view1, 'view2_image': curr_img_view2, 'view3_image': curr_img_view3, 'view4_image': curr_img_view4, 'label': curr_label}
...
for i in range(0,5):
	if i != val_fold:
		...
		for idx_4 in folds[i]:
			...
			x_train.append((all_features[idx_4[0],:,:,:],all_features[idx_4[1],:,:,:],all_features[idx_4[2],:,:,:],all_features[idx_4[3],:,:,:]))
		...
		for idx_4 in folds[i]:
			...
			x_val.append((all_features[idx_4[0],:,:,:],all_features[idx_4[1],:,:,:],all_features[idx_4[2],:,:,:],all_features[idx_4[3],:,:,:]))
			if entire_dataset==False and ctr==16:
				break
...
if 'v' in model_name:
	...
	model_ft = inception_v3_fourview_withdropoutcontrol.inception_v3_fourview_withdropoutcontrol(pretrained=pretrained, num_classes=num_classes, transform_input=transform_input)
...
if just_top_layer==True:
	if 'v' in model_name:
		...
		for p in model_ft.parameters():
			...
			w1 = p
			# pdb.set_trace()
			model_weights.extend(w1.view(-1).data.numpy())
			# curr_max = torch.max(w1).data[0]
			# curr_min = torch.min(w1).data[0]
			# if curr_max > max_val:
			# 	max_val = curr_max
			# if curr_min < min_val:
			# 	min_val = curr_min
...
hist_weights = np.histogram(model_weights,hist_nbins)
y_val = np.log10(1+hist_weights[0])
x_val = hist_weights[1]
plt.plot(0.5*(x_val[0:hist_nbins]+x_val[1:hist_nbins+1]),y_val)
plt.show()
...
# torch.manual_seed(37)
if 'i' in model_name:
	train_data = MyDataset_FourView(x_train,y_train,Rescale(299))
	val_data = MyDataset_FourView(x_val,y_val,Rescale(299))
elif 'v' in model_name:
	train_data = MyDataset_FourView(x_train,y_train)
	val_data = MyDataset_FourView(x_val,y_val)
...
def validate():
	...
	for data_val in val_loader:
		...
		view3_inputs = Variable(data_val['view3_image'].float().cuda(), requires_grad=False)
		view4_inputs = Variable(data_val['view4_image'].float().cuda(), requires_grad=False)
		labels = Variable(data_val['label'].long().cuda(), requires_grad=False)


		outputs = model_ft(view2_inputs,view1_inputs,view3_inputs,view4_inputs)
		del view2_inputs, view1_inputs, view3_inputs, view4_inputs
...
for epoch_cnt in range(epochs):
	...
	for data_train in train_loader:
		...
		view3_inputs = Variable(data_train['view3_image'].float().cuda(), requires_grad=False)
		view4_inputs = Variable(data_train['view4_image'].float().cuda(), requires_grad=False)
		labels = Variable(data_train['label'].long().cuda(), requires_grad=False)

		# outputs = model_ft(view2_inputs,view1_inputs,view3_inputs,view4_inputs,dropout_prob)
		# del view2_inputs, view1_inputs, view3_inputs, view4_inputs
